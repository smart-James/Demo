...
double Widget::Interest(int typeIn,int dateIn,int rateIn)
{
    double rate;
    if(typeIn == 0)
    {
...
/**************************************公积金贷款 值改变*******************************************************************/
    ...
    else
    {
        ...
        else
        {
            ...
            //2020.4.20
            case 0:rate = 2.75;break;
            //2020.3.20
            case 1:rate = 2.75;break;
            //2019.9.20...
            }
        }
    }
...
void Widget:: Clear()
{
    ...
    ui->lineEdit_rate->undo();
    ui->lineEdit_Rearea_1->clear();
    ui->lineEdit_Rerate_1->undo();
    ui->lineEdit_value->clear();

    ui->lineEdit_loan->clear();
    ui->lineEdit_rate_2->undo();
    ui->lineEdit_Rerate_2->undo();
    ui->lineEdit_Retotalloan->clear();

    ui->lineEdit_shangLoan->clear();
    ui->lineEdit_shangRate->undo();
    ui->lineEdit_ReshangRate->undo();
    ui->lineEdit_ReshangLoan->clear();
    ui->lineEdit_gongLoan->clear();
    ui->lineEdit_gongRate->undo();
    ui->lineEdit_RegongRate->undo();
    ui->lineEdit_RegongLoan->clear();

    ui->radioButton_benxi->setChecked(true);
    ui->radioButton_benxi_2->setChecked(true);
    ui->radioButton_benxi_3->setChecked(true);
    ui->radioButton_Rebenxi_1->setChecked(true);
    ui->radioButton_Rebenxi_2->setChecked(true);
    ui->radioButton_Rebenxi_3->setChecked(true);
...
/******************************************-----Page.NO.1.-----输入判断*******************************************************************/
    ...
    if(!checkValue.isEmpty()&&!checkArea.isEmpty())
    {
        ...
        {
            if(i==checkRate.length()-1)
            {
                if(checkRate.at(checkRate.length()-1)=='%')
                {
                    checkRate1=checkRate;
                    break;
                }
                else checkRate1=checkRate+"%";
            }
...
//        利率下拉框
        ...
        ui->lineEdit_Rerate_1->setText(checkRate1);
...
//        贷款类型下拉框
        ...
        Area_loan A(value,area,Percentage,Deadline,rate);
...
/******************************************-----Page.NO.1.-----计算方式选择********************************************************/
        ...
        {
            ui->radioButton_Rebenjin_1->setChecked(true);
...
//        利率下拉框
        ...
        ui->lineEdit_Rerate_2->setText(checkRate1);
...
//        贷款类型下拉框
        ...
        {
            ui->radioButton_Rebenjin_2->setChecked(true);;
...
void Widget::on_pushButton_start_3_clicked()
{
    ...
    if(!checkShangLoan.isEmpty()&&!checkGongLoan.isEmpty()&&!checkShangRate.isEmpty()&&!checkGongRate.isEmpty())
    {
        ...
        checkPoint = 0;
        int n = 0;
        for(int i= 0 ;i<checkShangRate.length();i++)
        {
            if(i==checkShangRate.length()-1)
            {
                if(checkShangRate.at(checkShangRate.length()-1)=='%')
                {
                    checkRate1=checkShangRate;
                    break;
                }
                else checkRate1=checkShangRate+"%";
            }
        ...
        checkPoint = 0;
        int k = 0;
        for(int i= 0 ;i<checkGongRate.length();i++)
        {
            if(i==checkGongRate.length()-1)
            {
                if(checkGongRate.at(checkGongRate.length()-1)=='%')
                {
                    checkRate2=checkGongRate;
                    break;
                }
                else checkRate2=checkGongRate+"%";
            }
...
//        商贷利率下拉框
        ...
        ui->lineEdit_ReshangRate->setText(checkRate1);

//        公积金贷款利率下拉框
        ui->comboBox_RegongRate->setCurrentIndex(ui->comboBox_gongRate->currentIndex());
        ui->lineEdit_RegongRate->setText(checkRate2);
...
//        贷款类型下拉框
        ...
        {
            ui->radioButton_Rebenjin_3->setChecked(true);
